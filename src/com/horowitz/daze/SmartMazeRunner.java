...
public class SmartMazeRunner {
	...
	private Position findGreenRecursive(Position pos, int row, int col) {
...
//	  	_mouse.mouseMove(p);
		...
		for (Position position : _matrix) {
			if (position.same(newPos)) {
				if (position._state == Status.UNKNOWN)
					continue;
				else if (position._state == Status.OBSTACLE) {//position._state == Status.WALKABLE || 
					canMove = false;
					break;
				}
			}
	...
	private Position lookForGreen(Position pos) throws RobotInterruptedException, IOException,
	    AWTException {
