...
public class Catalano {
	...
	private void applyFilterGate(String prefix, String sign, ColorFiltering colorFiltering, Channel channel) throws IOException {
		BufferedImage image1 = ImageIO.read(new File(prefix + ".bmp"));
		FastBitmap fb1 = new FastBitmap(image1);
...
//			ImageIO.write(fbXOR.toBufferedImage(), "BMP", new File("ocr/digitXOR" + prefix + ".bmp"));
//		} catch (IOException e) {
	...
	private FastBitmap processAND(String prefix, int numImages) {
		try {
			...
			for (int i = 2; i <= numImages; i++) {
				...
				//ImageIO.write(fb1.toBufferedImage(), "BMP", new File(prefix + i + ".bmp"));
			}
