...
public class GraphMazeRunner {
	...
	private boolean checkPopup() throws IOException, AWTException, RobotInterruptedException {
		...
		long start = System.currentTimeMillis();
		Rectangle area = _scanner.generateWindowedArea(514, 244);
		area.x = area.x + area.width - 60;
		area.y -= 18;
		area.width = 60;
		area.height = 60;
	...
	private boolean isGate(Pixel pp) throws RobotInterruptedException, AWTException, IOException {
		...
		_mouse.delay(100);
		Rectangle area = new Rectangle(pp.x + 17, pp.y + 12, 21 + 4, 12 + 8);
	...
	private BufferedImage filterGate(BufferedImage image) {
		...
		ColorFiltering colorFiltering = new ColorFiltering(new IntRange(45, 80), new IntRange(95, 255),
		    new IntRange(0, 120));
		colorFiltering.applyInPlace(fb1);
